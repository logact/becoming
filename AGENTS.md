...
- `src/ui/` — React Native UI layer: `composition/` (database startup, service-graph composition, `AppServicesProvider` context), `navigation/` (lightweight state-driven shell with Goals/Projects/Tasks tabs; no navigation library), `shared/` (design primitives: entity list scaffold, status badge, sheet, confirm dialog, toast), `goals/` (Goal planning screens: list, detail with pursuit-action slots for #134, New/Edit sheet), `projects/` (Project planning screens: list, detail with Overview/Structure/Progress segment slots for #135/#136 plus a membership-actions slot for #132, Overview, New/Edit sheet, and Goal-pursuit flows for both Goal and Project contexts), `tasks/` (Task planning screens: list, detail with lifecycle-inspect and membership actions, New/Edit sheet, and Task↔Project membership flows for both Task and Project Overview contexts). UI owns no domain logic; it consumes application services only.
