...
- Quick Capture deliberately remains compact: Goal and Task captures do not expose Start or Due and create unscheduled items. Scheduling is available from the resulting detail screen. This does not limit structured creation flows, which do expose scheduling where described below.
...
### Scheduling and date entry

- Goal and Task detail headers show Start and Due together, including **No schedule** when neither exists and an explicit not-set label when only one exists. **Schedule** opens a focused editor with optional native Start and Due rows. Either value can be selected, changed, or cleared; reciprocal picker bounds enforce Start on or before Due, with domain validation as the final guard.
- Saving a schedule keeps one atomic Start/Due draft, blocks duplicate submission, closes the editor on success, refreshes detail, and reveals the immutable schedule-change activity. Cancel is lossless. A failed save keeps the selected values and editor open with a concise error. Scheduling never changes lifecycle state or the labels of explicit Start/Resume actions.
- Structured Goal and Task creation accepts optional Start and Due in **Create from this idea** and in **Add to plan** for Sub-goals and Tasks. The same native control supplies the required Milestone date, Project Due, and local start/end date-times for resource allocation. Quick Capture remains the unscheduled fast path.
- A `todo` Goal or Task whose Start day has arrived is shown as **Ready to start** in Dashboard, Goals, and Tasks Needs attention. Higher-priority failed and due-related warnings still win, each entity appears only once, and ready items order oldest Start first. These items remain `todo` until the user invokes Start.
...
1. dashboard  page show 
   ...
   the attention section combines built-in rules (failed items, approaching overdue — goal/project 1 day, task 2 h — projects whose quantity resource is >=90% consumed, and scheduled todo Goals/Tasks ready to start) with user control: the user can pin any item into the section and remove (dismiss) items from it.
...
2. goals page is a dashboard for all goals, not only a goal list:
   ...
   2. Needs attention (failed goals, goals due within 1 day, then todo goals ready to start)
...
3. tasks page is the dashboard for all tasks:
   ...
   2. Needs attention ordered by failed, overdue, due soon, then todo tasks ready to start
